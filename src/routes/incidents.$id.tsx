...
function IncidentDetail() {
  ...
  const line = series.map((v, i) => `${(i * step).toFixed(1)},${(h - (v / max) * h).toFixed(1)}`).join(" ");
      ...
      <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <Button asChild variant="ghost" size="sm">
            <Link to="/incidents"><ArrowLeft className="mr-1 h-4 w-4" /> All incidents</Link>
          </Button>
          {incident.status === "active" && (
            <Button onClick={resolve}><CheckCircle2 className="mr-1 h-4 w-4" /> Mark resolved</Button>
          )}
        </div>
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex flex-wrap items-start justify-between gap-4">
              <div>
                <div className="text-xs uppercase tracking-widest text-muted-foreground">Incident</div>
                <h1 className="font-display text-3xl font-semibold capitalize">{incident.trigger.replace("-", " ")}</h1>
                <p className="mt-1 text-sm text-muted-foreground">{new Date(incident.startedAt).toLocaleString()}</p>
              </div>
            ...
            <div className="mt-6 grid gap-4 sm:grid-cols-3">
              ...
              <Stat label="Contacts reached" value={`${new Set(incident.alerts.map((a) => a.contactId)).size}`} />
            </div>
          </CardContent>
        </Card>
        ...
        <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
          <Card>
            <CardHeader><CardTitle className="text-base">Signal trace</CardTitle></CardHeader>
            <CardContent>
              <svg viewBox={`0 0 ${w} ${h}`} className="h-24 w-full" preserveAspectRatio="none">
                <polyline points={line} fill="none" stroke="var(--color-destructive)" strokeWidth="2" />
              </svg>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            {incident.alerts.length === 0 ? (
              <p className="text-sm text-muted-foreground">No contacts configured — add some on the Contacts page.</p>
            ) : (
              incident.alerts
                .slice()
                .sort((a, b) => a.at - b.at)
                .map((a, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex items-center gap-3 rounded-lg border border-border bg-card/50 px-3 py-2">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="text-xs text-muted-foreground capitalize">
                          {a.channel === "inapp" ? "In-app push" : a.channel} · {new Date(a.at).toLocaleTimeString()}
                        </div>
                      </div>
            ...
            <p className="mt-3 text-xs text-muted-foreground">Simulated delivery for prototype purposes.</p>
          </CardContent>
        </Card>
      </main>
    </div>
