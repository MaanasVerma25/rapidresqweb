    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-5xl space-y-6 px-4 py-8 sm:px-6">
        <div>
          ...
          <p className="mt-1 text-muted-foreground">A record of every alert RapidResQ has triggered.</p>
        </div>
        ...
        {incidents.length === 0 ? (
          <Card>
            <CardContent className="flex flex-col items-center gap-3 p-12 text-center">
              ...
              <p className="text-sm text-muted-foreground">Trigger a scenario from the dashboard to see how it appears here.</p>
            </CardContent>
          </Card>
        ) : (
                ...
                <Card className="transition-shadow hover:shadow-soft">
                  <CardContent className="flex items-center gap-4 p-4">
                    <div className="grid h-11 w-11 place-items-center rounded-xl bg-destructive/10 text-destructive">
                      <span className="font-display text-lg font-semibold">{Math.round(i.peakScore * 100)}</span>
                    </div>
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        <span className="font-medium capitalize">{i.trigger.replace("-", " ")}</span>
                        <Badge variant={i.status === "active" ? "destructive" : "secondary"}>{i.status}</Badge>
                      </div>
